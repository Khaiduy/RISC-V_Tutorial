/* EDHOC Initiator - Board A - Suite 0/1, Method 1
 *
 ...
 *
 * Authentication: Initiator uses Ed25519 signature key (SK); Responder uses static DH (SDHK)
...
// Ed25519 Authentication Keys (Initiator - SK)
...
// Responder's STATIC DH public key (G_R) - hardcoded (X25519(r_sk, basepoint))
// Must match responder's computed g_r
...
int main(void) {
    ...
    // Responder's DH key (g_r) is hardcoded constant — no computation needed.
    {
        uint8_t seed_tmp[32];
        memcpy(seed_tmp, i_sign_seed, 32);
        crypto_ed25519_key_pair(i_sign_sk, i_sign_pk, seed_tmp);
    }
    ...
    cred_i_len = build_ccs_credential(cred_i, "InitM1", kid_i, 1, i_sign_pk, 32, 6);
    ...
    // Signature keys (Method 1) - Initiator's long-term Ed25519 keys
