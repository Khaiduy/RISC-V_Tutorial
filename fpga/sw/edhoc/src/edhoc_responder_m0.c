...
volatile uint32_t __attribute__((weak)) _tb_msg2_cyc = 0;
volatile uint32_t __attribute__((weak)) _tb_msg3proc_cyc = 0;
...
int main(void) {
   ...
   // Static DH public key
    ...
    cred_i_len = build_ccs_credential(cred_i, "InitM3", kid_i, 1, g_i, 32, 6);
    ...
    kprintf("\r\nMethod 0: authentication (both parties)\r\n");
#endif
    ...
    kprintf("DEBUG right before edhoc_responder_run: ctx_r.id_cred_r.len = %d\n", ctx_r.id_cred_r.len);

    static struct edhoc_responder_context ctx_r_copy;
    memcpy(&ctx_r_copy, &ctx_r, sizeof(struct edhoc_responder_context));
    kprintf("DEBUG right before edhoc_responder_run with copy: ctx_r_copy.id_cred_r.len = %d\n", ctx_r_copy.id_cred_r.len);
    enum err result = edhoc_responder_run(&ctx_r_copy, &cred_i_array, &err_msg, &prk_out,
