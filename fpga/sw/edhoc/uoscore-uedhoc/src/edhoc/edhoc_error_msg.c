...
static uint32_t cbor_enc_int(uint8_t *buf, uint32_t cap, int val)
{
	if (val >= 0) {
		...
		/* Negative: encode as 1+|n| as unsigned, with major type 1. */
			      ...
			      const char *s, uint32_t len)
{
	...
	if (len <= 23) {
		...
		return 0; /* unrealistic for diag */
	}
	memcpy(buf + hdr, s, len);
	return hdr + len;
}

/* Decode an int (signed). Returns 0 on failure, otherwise bytes consumed. */
static uint32_t cbor_dec_int(const uint8_t *buf, uint32_t len, int *out)
{
	...
	if (ai <= 23) {
		...
		return 0; /* 64-bit int or invalid */
	}
...
bool edhoc_is_error_message(const uint8_t *buf, uint32_t len)
{
	...
	uint8_t mt = buf[0] >> 5;
	/* CBOR major type 0 (uint) or 1 (negint) at the start of the
	 * sequence => error message. msg_2/3/4 start with bstr (mt=2). */
	return (mt == 0 || mt == 1);
}
