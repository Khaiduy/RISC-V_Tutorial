...
*/
/* Legacy crypto wrapper: Monocypher + TinyCrypt + (optionally) Ascon-c.
 * Resurrected from commit 1556c51c (pre-wolfSSL era) to provide a faster,
 * smaller backend for Suites 0/1/4/5/7 + PSK Method 4. Mutually exclusive
 * with crypto_wrapper.c (wolfSSL) via the outer-guard pattern: only the
 * file matching the active backend define emits symbols. */
