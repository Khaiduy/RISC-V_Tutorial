...
{
	zcbor_log("%s\r\n", __func__);
	// kprintf("[CBOR] Encoding edhoc_enc_structure\r\n");
	...
	if (!tmp_result) {
		...
		// kprintf("[CBOR] %s error: %d\r\n", __func__, zcbor_peek_error(state));
		// kprintf("[CBOR] Encoding edhoc_enc_structure failed\r\n");
	} else {
		zcbor_log("%s success\r\n", __func__);
	}
...
{
	// kprintf("[CBOR] %s called\r\n", __func__);
	// kprintf("[CBOR] payload ptr=%d, payload_len=%d\r\n", (int)payload, payload_len);
	// kprintf("[CBOR] input ptr=%d\r\n", (int)input);
	// kprintf("[CBOR] payload_len_out ptr=%d\r\n", (int)payload_len_out);
