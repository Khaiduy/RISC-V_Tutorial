		  ...
		  struct byte_array *c_i)
{
	...
	if (!(selected_suite_is_supported(suites_i.ptr[suites_i.len - 1],
		...
		/* Surface a real error code; run_extended emits the RFC §6 wire-format
		 * error message with ERR_CODE=2 (Wrong cipher suite) + SUITES_R. */
...
/* Send a pre-built CBOR wire-format error message; copy into user err_msg buf. */
static void psk_send_and_store_error(void *sock,
				     enum err (*tx)(void *, struct byte_array *),
...
/* Compute-only cycle counters for PSK responder; exclude UART. */
volatile uint32_t _tb_psk_resp_msg2_cyc  = 0;  /* incl. msg1 parse */
volatile uint32_t _tb_psk_resp_msg3_cyc  = 0;
volatile uint32_t _tb_psk_resp_msg4_cyc  = 0;
volatile uint32_t _tb_psk_resp_total_cyc = 0;
	...
	enum err (*ead_process)(void *params, struct byte_array *ead13))
{
	...
	/*receive message 1 — listening, no error msg to send yet */
#ifdef DEBUG_PRINT
	kprintf("[R] RX msg1...\r\n");
#endif
	r = rx(c->sock, &rc.msg);
	...
	/*process msg1 and generate message 2 — suite-mismatch path emits ERR_CODE=2 */
#ifdef DEBUG_PRINT
	...
	if (r != ok) {
		kprintf("[R] msg2_gen FAILED: %d\r\n", r);
		if (r == unsupported_cipher_suite) {
			uint8_t info[16];
			uint32_t info_len = encode_suites_cbor(info, sizeof(info),
							      &c->suites_r);
			if (info_len > 0 &&
			    edhoc_build_error_message(ebuf, sizeof(ebuf),
				EDHOC_ERR_CODE_WRONG_CIPHER_SUITE,
				NULL, 0, info, info_len, &ebuf_len) == ok)
				psk_send_and_store_error(c->sock, tx, err_msg,
							 ebuf, ebuf_len);
		} else {
			if (edhoc_build_error_message(ebuf, sizeof(ebuf),
				EDHOC_ERR_CODE_UNSPECIFIED,
				"EDHOC-PSK responder message_2 generation failed",
				0, NULL, 0, &ebuf_len) == ok)
				psk_send_and_store_error(c->sock, tx, err_msg,
							 ebuf, ebuf_len);
		}
		return r;
	...
	/*process msg3 — draft-ietf-lake-edhoc-psk-07 §9 mandates indistinguishable
	 * failure handling: ANY failure here (AEAD, unknown credential, malformed
	 * input) MUST produce the same observable outcome. */
#ifdef DEBUG_PRINT
	...
	if (r != ok) {
		kprintf("[R] msg3_process FAILED: %d\r\n", r);
		/* PSK uniform-error per draft-07 §9: always emit the SAME wire
		 * message regardless of which step failed (AEAD vs cred vs CBOR). */
		if (edhoc_build_error_message_psk_uniform(ebuf, sizeof(ebuf),
							  &ebuf_len) == ok)
			psk_send_and_store_error(c->sock, tx, err_msg, ebuf, ebuf_len);
		return r;
	}
	r = ead_process(c->params_ead_process, &rc.ead);
	__asm__ volatile ("rdcycle %0" : "=r"(t_end));
	...
	if (r != ok) {
		kprintf("[R] ead_process FAILED: %d\r\n", r);
		if (edhoc_build_error_message_psk_uniform(ebuf, sizeof(ebuf),
							  &ebuf_len) == ok)
			psk_send_and_store_error(c->sock, tx, err_msg, ebuf, ebuf_len);
		return r;
	...
	_tb_psk_resp_msg4_cyc = (uint32_t)t_msg4_gen;
#else
	uint64_t t_msg4_gen = 0;
#endif // MESSAGE_4

	_tb_psk_resp_msg2_cyc  = (uint32_t)t_msg2_gen;
	_tb_psk_resp_msg3_cyc  = (uint32_t)t_msg3_process;
	_tb_psk_resp_total_cyc = (uint32_t)(t_msg2_gen + t_msg3_process + t_msg4_gen);
