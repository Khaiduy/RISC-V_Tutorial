	...
	struct byte_array *plaintext)
{
	...
	TRY(ciphertext_encrypt_decrypt(ctxt == CIPHERTEXT2, ENCRYPT, plaintext, &key, &iv,
				       &aad, ciphertext, &tag));
	/* AEAD writes tag to a separate buffer; append it to ciphertext so the
	 * wire bytes are <ciphertext || tag>. Required for empty-plaintext
	 * cases (PSK msg_4 with no EAD_4). */
	if (ctxt != CIPHERTEXT2 && tag.len > 0) {
		for (uint32_t i = 0; i < tag.len; i++) {
			ciphertext->ptr[ciphertext->len + i] = tag.ptr[i];
		}
	}
	...
	struct byte_array *plaintext)
{
	...
	if (plaintext_3b.len > 0) {
		...
		/* Empty plaintext: AEAD produces only the authentication tag.
		 * Compute tag into tag_3b, then copy into ciphertext_3b so the
		 * wire payload contains the 16-byte tag (RFC 9528 §5.4.2). */
		ciphertext_3b.len = 0;
		TRY(aead(ENCRYPT, &plaintext_3b, &k_3, &iv_3, &external_aad,
			         &ciphertext_3b, &tag_3b));
		for (uint32_t i = 0; i < tag_len; i++) ciphertext_3b.ptr[i] = tag_3b.ptr[i];
