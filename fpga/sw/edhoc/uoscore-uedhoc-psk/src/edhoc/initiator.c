...
/* Compute-only cycle counters, populated by edhoc_initiator_run_extended.
 * Exclude all UART tx/rx wait time. Apps read these directly. */
volatile uint32_t _tb_psk_init_msg1_cyc  = 0;
volatile uint32_t _tb_psk_init_msg3_cyc  = 0;  /* incl. msg2 parse */
volatile uint32_t _tb_psk_init_msg4_cyc  = 0;
volatile uint32_t _tb_psk_init_total_cyc = 0;
	...
	enum err (*ead_process)(void *params, struct byte_array *ead24))
{
	...
	uint64_t t_msg1_gen = 0, t_msg3_gen = 0, t_msg4_proc = 0;
	enum err r;
	uint8_t ebuf[MSG_MAX_SIZE];
	uint32_t ebuf_len = 0;

	/*create and send message 1 — local failure, nothing to send on wire */
	...
	/*create and send message 3 — draft-ietf-lake-edhoc-psk-07 §9 mandates
	 * indistinguishable handling of msg_3-path failures (including those
	 * arising from msg_2 processing where the responder's PSK-encrypted
	 * credential is decoded). Use uniform-error helper on any failure. */
	__asm__ volatile ("rdcycle %0" : "=r"(t_start));
	...
	if (r != ok) {
		if (edhoc_build_error_message_psk_uniform(ebuf, sizeof(ebuf),
							  &ebuf_len) == ok)
			(void)tx(c->sock, &(struct byte_array){ .ptr = ebuf, .len = ebuf_len });
	...
	r = msg4_process(&rc);
	__asm__ volatile ("rdcycle %0" : "=r"(t_end));
	t_msg4_proc = t_end - t_start;
	if (r != ok) return r;
	r = ead_process(c->params_ead_process, &rc.ead);
	if (r != ok) return r;
#endif // MESSAGE_4

	_tb_psk_init_msg1_cyc  = (uint32_t)t_msg1_gen;
	_tb_psk_init_msg3_cyc  = (uint32_t)t_msg3_gen;
	_tb_psk_init_msg4_cyc  = (uint32_t)t_msg4_proc;
	_tb_psk_init_total_cyc = (uint32_t)(t_msg1_gen + t_msg3_gen + t_msg4_proc);
